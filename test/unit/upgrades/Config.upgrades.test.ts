...
import {
    ...
    ConfigNewImplementation,
    ConfigNewImplementation__factory
} from '../../../typechain-types'

describe('Config Upgrade Tests', () => {
    ...
    it('should not allow upgrade with empty data', async () => {
        const timelockAddress = await addressBook.timelock()
        const currentVersion = await config.implementationVersion()
        const uniqueId = await config.uniqueContractId()
        const newImplementation = await new ConfigNewImplementation__factory(owner).deploy(currentVersion + 1n, timelockAddress, uniqueId)
        await newImplementation.waitForDeployment()

        await expect(
            config.connect(governance).upgradeToAndCall(
    ...
    it('should allow upgrade to version+1', async () => {
        ...
        const tx = await config.connect(governance).upgradeToAndCall(
            await newImplementation.getAddress(),
            initData
        )
        await tx.wait()

        expect(await config.implementationVersion()).to.equal(currentVersion + 1n)
    })
    ...
    it('should allow upgrade to version+100', async () => {
        ...
        const currentVersion = await config.implementationVersion()
        const uniqueId = await config.uniqueContractId()
        const newImplementation = await new ConfigNewImplementation__factory(owner).deploy(currentVersion + 100n, timelockAddress, uniqueId)
        await newImplementation.waitForDeployment()
        ...
        expect(await config.implementationVersion()).to.equal(currentVersion + 100n)
    })
    ...
    it('should not allow upgrade to version+101', async () => {
        ...
        const currentVersion = await config.implementationVersion()
        const uniqueId = await config.uniqueContractId()
        const newImplementation = await new ConfigNewImplementation__factory(owner).deploy(currentVersion + 101n, timelockAddress, uniqueId)
        await newImplementation.waitForDeployment()
    ...
    it('should not allow upgrade to the same version', async () => {
        ...
        const currentVersion = await config.implementationVersion()
        const uniqueId = await config.uniqueContractId()
        const newImplementation = await new ConfigNewImplementation__factory(owner).deploy(currentVersion, timelockAddress, uniqueId)
        await newImplementation.waitForDeployment()
    ...
    it('should not allow upgrade from not upgrade role', async () => {
        ...
        await expect(
            config.connect(user).upgradeToAndCall(
                await newImplementation.getAddress(),
                newImplementation.interface.encodeFunctionData('initialize')
            )
    ...
    it('should not allow upgrade with wrong uniqueContractId', async () => {
        const timelockAddress = await addressBook.timelock()
        const currentVersion = await config.implementationVersion()
        const wrongUniqueId = ethers.keccak256(ethers.toUtf8Bytes("WrongConfig"))
        const newImplementation = await new ConfigNewImplementation__factory(owner).deploy(currentVersion + 1n, timelockAddress, wrongUniqueId)
        await newImplementation.waitForDeployment()
